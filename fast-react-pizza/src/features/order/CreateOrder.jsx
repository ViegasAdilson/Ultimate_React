...
function CreateOrder() {
  ...
  const isLoadinggAddress = addressStatus === 'loading'
    ...
    <div className='py-6 px-4'>
      ...
      <Form method="POST" action="/order/new">
        ...
        <div className='relative mb-5 flex gap-2 flex-col sm:flex-row sm:items-center'>
          ...
          <div className='grow'>
            <input type="text" name="address" defaultValue={address} disabled={isLoadinggAddress} required className="input w-full" />
            {addressStatus === 'error' && <p className='text-xs mt-2 text-red-700 bg-red-100 p-2'>{errorAddress}</p>}
          </div>
          {!position.latitude && !position.longitude &&  <span className='md:right-[5px] md:top-[5px] top-[3px] absolute right-[3px] z-50'>

              <Button type='small' disabled={isLoadinggAddress} onClick={(e)=> {
              e.preventDefault()
        ...
        <div>
          ...
          <Button disabled={isSubmitting} type='primary'>
            {isSubmitting ? 'Placin order...' : `Order now for ${formatCurrency(totalPrice)}`}
          </Button>
        </div>
      </Form>
    </div>
