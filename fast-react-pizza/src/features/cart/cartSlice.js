...
const cartSlice = createSlice({
  ...
  reducers:{
    ...
    increaseItemQuantitty(state, action){
      const item = state.cart.find((item )=> item.pizzaId === action.payload)
      item.quantity++;
      item.totalPrice = item.quantity * item.unitPrice;
    },
    decreaseItemQuantitty(state, action){
      const item = state.cart.find((item )=> item.pizzaId === action.payload)
      item.quantity--;
      item.totalPrice = item.quantity * item.unitPrice;
...
export const {addItem, deleteItem, decreaseItemQuantitty, increaseItemQuantitty, clearCart} = cartSlice.actions;
